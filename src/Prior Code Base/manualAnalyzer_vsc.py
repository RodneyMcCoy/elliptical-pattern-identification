...
flightData = r"C:\Users\Colleen\ChilePythonEnvironment\ChileData_12192020\Tolten"
fileToBeInspected = 'T1_1600_121320_ACE.txt'
microHodoDir = r"C:\Users\Colleen\ChilePythonEnvironment\hodoAnalysis\testHodos"     #location where selections from siftThroughUV are saved. This is also the location where do analysis looks for micro hodos to analysis
waveParamDir = r"C:\Users\Colleen\ChilePythonEnvironment\hodoAnalysis\testWaveParams_csv"

if location == "Tolten":
    latitudeOfAnalysis = -39.236248 * units.degree     #latitude at which sonde was launched. Used to account for affect of coriolis force.
elif location == "Villarica":
    latitudeOfAnalysis = -39.30697 * units.degree     #latitude at which sonde was launched. Used to account for affect of coriolis force.
...
def manualTKGUI():
    ...
    class App:
        ...
        def update(self, *args):
            ...
            print("SliderAlt: ", sliderAlt)
            print("Alt.mag: ", Alt.magnitude)
            print("altType: ", type(Alt.magnitude[0]))
            print("SliderAltType:", type(sliderAlt))


            print("npWhere:", np.where(Alt.magnitude == sliderAlt)[0][0])
            print("minSliderAlt: ", min(Alt.magnitude.tolist()))


            sliderWindow = int(self.winSpinner.get())
            print("sliderWindow: ", sliderWindow)
...
def doAnalysis(microHodoDir):
    ...
    print("microPathExists: ", os.path.exists(microHodoDir))
    ...
    for file in os.listdir(microHodoDir):
        ...
        instance.addNameAddPath(file, microHodoDir)  #file name added to object attribute here to be used in labeling plots
...
def preprocessDataNoResample(file, path):
    ...
    uMean = signal.savgol_filter(u.magnitude, window, 3, mode='mirror') * units.m/units.second
    vMean = signal.savgol_filter(v.magnitude, window, 3, mode='mirror') * units.m/units.second
    tempMean = signal.savgol_filter(Temp.magnitude, window, 3, mode='mirror') * units.degC
...
class microHodo:
    ...
    def saveMicroHodoNoIndices(self):
        ...
        T.to_csv('{}\{}.csv'.format(self.savepath, fname), index=False, float_format='%4.3f')
